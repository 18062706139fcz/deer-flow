...
Browser login uses `HttpOnly` session cookies. The login page offers a "keep me signed in" option that extends the browser session when the request is HTTPS (including trusted `X-Forwarded-Proto: https`) or localhost HTTP. Public HTTP deployments, including many temporary sandbox URLs, fall back to session cookies by default. DeerFlow never stores the password in browser storage; the UI may remember only the email address.

If you expose the Gateway directly without the bundled nginx, make sure clients cannot spoof `Forwarded` / `X-Forwarded-*` headers. DeerFlow's same-origin and secure-cookie decisions use those headers to recover the browser-facing origin behind a trusted proxy.
